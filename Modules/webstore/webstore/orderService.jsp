...
<!--      <link href="https://fonts.googleapis.com/css?family=Lato:400,400i,700,900&display=swap" rel="stylesheet"> -->
   ...
   var resData = "";
	function generateLoad(){
		...
		var downloadBytes =  Math.abs(document.getElementById('loopSec1').value);
		var requestCount =  Math.abs(document.getElementById('loopSec2').value);
		if(!Number.isNaN(duration)){
			...
			for(i =1;i<=requestCount;i++){
				...
				document.getElementById('cpu-looping').innerHTML = "<div style='display:inline-block'>"+ "Sending Request To Service Details IN-PROGRESS ...." + "</div>";
				var url = "ordermakerservice?sleepTime="+duration+"&downloadBytes="+downloadBytes;
				request.onreadystatechange = function() {
					if (this.readyState === 4 && this.status === 200) {
						document.getElementById('cpu-looping').style.display = "none";
						document.getElementById('loopSec').value = "";
						document.getElementById('cpu-looping').innerHTML = "<div style='display:inline-block'><div class='tick' style='display:inline-block'></div>"+ "Sending Request To Service Details Finished !!!" + "</div>";
...
<form class="form-signin">
  <div class="text-left mb-4">
    ...
    <h1 class="h3 mb-2 font-weight-normal">Ajax File Download</h1>
    <p>Enter Sleep Time For Each Service Request</p>
  </div>

  <div class="form-label-group">
    <input  id="loopSec" name="loopSec" class="form-control" type="number" placeholder="Milliseconds" required autofocus>
    <label for="loopSec">milliseconds</label>
  </div>
  <div class="form-label-group1">
  	<p>Enter Download Size In Bytes For Each Service Request</p>
    <input  id="loopSec1" name="loopSec1" class="form-control" type="number" placeholder="download bytes size" required autofocus>
  </div>
  <div class="form-label-group2">
  	<p>Enter Number Of Ajax Service Request To Send</p>
    <input  id="loopSec2" name="loopSec2" class="form-control" type="number" placeholder="request count" required autofocus>
	
  </div>
	<div class="form-label-group">
		<div class="loading" id="cpu-looping" style="display:none">Sending Request To Service Details IN-PROGRESS .... </div>
	</div>
	<div class="form-label-group">
		<div class="noloading" id="cpu-done" style="display:none"><div class='tick' style='display:inline-block'></div>Sending Request To Service Details Finished !!! </div>
	</div>
