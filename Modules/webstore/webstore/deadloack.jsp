...
<!--      <link href="https://fonts.googleapis.com/css?family=Lato:400,400i,700,900&display=swap" rel="stylesheet"> -->
   ...
   <script>
	function generateLoad(){
		...
		document.getElementById('cpu-looping').innerHTML = "<div style='display:inline-block'>"+ "DeadLock Sceanrios IN-PROGRESS ...." + "</div>";
		
		var url = "productController/getExpireProduct";
		...
		request.onreadystatechange = function() {
			...
			if (this.readyState === 4 && this.status === 200) {
				...
				document.getElementById('cpu-looping').innerHTML = "<div style='display:inline-block'><div class='tick' style='display:inline-block'></div>"+ "Slow Code Generation Finished !!!" + "</div>";
...
<form class="form-signin">
  <div class="text-left mb-4">
    ...
    <h1 class="h3 mb-2 font-weight-normal">Generate Java Slowness</h1>
  </div>

  <div class="form-label-group">
    <input  id="loopSec" name="loopSec" class="form-control" type="number" placeholder="Seconds" required autofocus>
    <label for="loopSec">Loop Count</label>
  </div>
	<div class="form-label-group">
		<div class="loading" id="cpu-looping" style="display:none">DeadLock Scenarios IN-PROGRESS .... </div>
	</div>
	<div class="form-label-group">
		<div class="noloading" id="cpu-done" style="display:none"><div class='tick' style='display:inline-block'></div>DeadLock Scenario Finished RESTART JVM !!! </div>
	</div>
